...
export const fetchWomenImages = async (req, res) => {
    try {
        const listParams = {
            Bucket: process.env.AWS_WOMEN_BUCKET_NAME,
            Prefix: 'WomensCollection/' // Root folder
        };
        ...
        if (response.Contents) {
            response.Contents.forEach(item => {
                ...
                const folderMatch = key.match(/WomensCollection\/([^/]+)\//);
                if (folderMatch) {
                    ...
                    allImages[folder].push(`https://${process.env.AWS_WOMEN_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${key}`);
                }
            });
        }
